...
const CustomReceiver = require('../apps/CustomReceiver/CustomReceiver')

const SUPPORTED_APPS = {
    [Youtube.APP_ID]: Youtube,
    [DefaultMediaReceiver.APP_ID]: DefaultMediaReceiver,
    [CustomReceiver.APP_ID]: CustomReceiver
}
...
class Device extends EventEmitter {
    ...
    _playCustomReceiver(callback) {
        this._launch(CustomReceiver, (err, player) => {
            if (err) return callback(err)
    ...
    play(resource, type, opts, callback) {
        ...
        this._connect((err) => {
            ...
            switch (type) {
                ...
                case "youtube":
                    ...
                    break
                case "customreceiver":
                    this._playCustomReceiver(callback)
                    break
                default:
                    callback(new Error('Unknown resource type'))
            }
        })
    }
    ...
    sendMessage(message, callback) {
        ...
        this._tryJoin((err) => {
            ...
            this.player.sendMessage(message, callback)
        })
    }
